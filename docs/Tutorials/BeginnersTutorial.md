...
pointing in the `-z` direction. This is because out BBH domain rotates about the
...
over time. Inside the container make a new directory `runs2` where you will run
it. Also copy the default input file in
...
out build directory. However, since the executables are already pre-built, this
