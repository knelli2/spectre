...
def _control_system_params(
    ...
    params = {}
    params["MaxDampingTimescale"] = max_damping_timescale
    params["KinematicTimescale"] = damping_time_base * total_mass
    params["SizeATimescale"] = damping_time_base * 0.2 * mass_right
    params["SizeBTimescale"] = damping_time_base * 0.2 * mass_left
    params["ShapeATimescale"] = 5.0 * params["KinematicTimescale"]
    params["ShapeBTimescale"] = 5.0 * params["KinematicTimescale"]

    params["SizeIncreaseThreshold"] = 1e-3
    params["DecreaseThreshold"] = (
        0.1 * decrease_threshold_base / (mass_ratio + 1.0 / mass_ratio)
    )
    increase_threshold_fraction = 0.25
    params["IncreaseThreshold"] = (
        increase_threshold_fraction * params["DecreaseThreshold"]
    )

    params["SizeBMaxTimescale"] = 10 if spin_magnitude_left > 0.9 else 20
    params["SizeAMaxTimescale"] = 10 if spin_magnitude_right > 0.9 else 20
...
def inspiral_parameters(
    ...
    assert (
        len(left_object.keys()) == 1
    ), "Expected only one option for 'ObjectLeft'"
    assert (
        len(right_object.keys()) == 1
    ), "Expected only one option for 'ObjectRight'"
    ...
    assert left_object_type in available_solutions, (
        f"Unknown solution for 'ObjectLeft': '{left_object_type}'. Known"
        f" solutions are: {available_solutions}"
    )
    assert right_object_type in available_solutions, (
        f"Unknown solution for 'ObjectRight': '{right_object_type}'. Known"
        f" solutions are: {available_solutions}"
    )
