...
void test_state_history(const size_t num_times_to_store) {
  ...
  std::vector<double> times(num_times_to_store);
  ...
  times.push_back(double(num_times_to_store));
  state_history.store(times[num_times_to_store], info, control_error_args);
  for (const size_t state : states) {
    const auto history = state_history.state_history(state);
    CHECK(history.size() == num_times_to_store);
    CHECK(history.front().first == times[1]);
    CHECK(history.back().first == times[num_times_to_store]);
  }
}
}  // namespace
