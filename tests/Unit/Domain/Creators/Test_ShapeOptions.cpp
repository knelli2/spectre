...
#include "Domain/Creators/ShapeOptions.hpp"
...
void test_ylms_from_file() {
  ...
  CHECK(
      ylms_from_file.subfile_names ==
      std::array<std::string, 3>{"Ylm_coefs", "dt_Ylm_coefs", "d2t_Ylm_coefs"});
...
void test_construct_funcs(const gsl::not_null<Generator*> generator) {
  ...
  // We choose a Schwarzschild BH so all coefs are zero and it's easy to check
  {
    ...
    const auto [shape_funcs, size_funcs] = domain::creators::
        time_dependent_options::construct_initial_shape_and_size_funcs(
  ...
  }
  {
    ...
    const std::array<std::string, 3> subfile_names{"Ylm_coefs", "dt_Ylm_coefs",
                                                   "d2t_Ylm_coefs"};
    ...
    // Scoped to close h5 file
    {
      ...
      for (size_t i = 0; i < 3; i++) {
        ...
        auto& file =
            test_file.insert<h5::Dat>("/" + gsl::at(subfile_names, i), legend);
        file.append(data);
    ...
    for (size_t i = 0; i < shape_funcs.size(); i++) {
      ...
      // Loop pointwise so we only check the coefficients that matter. Also
...
SPECTRE_TEST_CASE("Unit.Domain.Creators.ShapeOptions", "[Domain][Unit]") {
  MAKE_GENERATOR(generator);
  test_kerr_schild_boyer_lindquist();
  test_ylms_from_file();
  test_shape_map_options();
  test_construct_funcs(make_not_null(&generator));
}
