...
void test(const bool include_expansion, const bool include_rotation,
  ...
  if (include_shape_a) {
    shape_map_a_options =
        IsCylindrical ? ShapeMapAOptions<IsCylindrical>{l_max_A, size_A_values}
  ...
  if (include_shape_b) {
    shape_map_b_options =
        IsCylindrical ? ShapeMapBOptions<IsCylindrical>{l_max_B, size_B_values}
                      : ShapeMapBOptions<IsCylindrical>{
                            l_max_B, size_B_values, false, Falloff::Inverse};
  }
  ...
  if (include_shape_a and not IsCylindrical) {
    CHECK_THROWS_WITH((ShapeMapAOptions<IsCylindrical>{l_max_A, size_A_values,
                                                       true, Falloff::Inverse}),
                      Catch::Matchers::ContainsSubstring(
                          "If the shape map transition ends at the cube, then "
                          "the falloff must be 'Linear'."));
  }
  if (include_shape_b and not IsCylindrical) {
    CHECK_THROWS_WITH((ShapeMapBOptions<IsCylindrical>{l_max_B, size_B_values,
                                                       true, Falloff::Inverse}),
                      Catch::Matchers::ContainsSubstring(
                          "If the shape map transition ends at the cube, then "
                          "the falloff must be 'Linear'."));
