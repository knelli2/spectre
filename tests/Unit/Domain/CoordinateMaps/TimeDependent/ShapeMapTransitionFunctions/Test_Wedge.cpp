...
void test_gradient() {
  INFO("Test gradient");
  const double overall_inner_radius = 0.5;
  const double overall_outer_radius = 10.0;
  const OrientationMap<3> orientation_map{};

  const auto make_wedge = [&](const double inner_sphericity,
                              const double outer_sphericity) {
    return Wedge{
        overall_inner_radius, overall_outer_radius, inner_sphericity,
        outer_sphericity,     overall_inner_radius, overall_outer_radius,
        inner_sphericity,     outer_sphericity,     orientation_map};
  };
  ...
  const auto compute_gradient =
      [&](const double radius, const std::array<double, 3>& point,
        ...
        const std::array<double, 3> inner_grad =
            extra_inner_grad_factor * overall_inner_radius * grad_base;
        const std::array<double, 3> outer_grad =
            extra_outer_grad_factor * overall_outer_radius * grad_base;
  ...
  const auto test_z_axis = [&](const double inner_sphericity,
    ...
    double inner_distance = overall_inner_radius;
    double outer_distance = overall_outer_radius;
  ...
  const auto test_corners = [&](const double inner_sphericity,
    ...
    for (const double radius :
         {overall_inner_radius, overall_outer_radius,
          0.5 * (overall_outer_radius + overall_outer_radius)}) {
      for (const double phi :
        ...
        CHECK_ITERABLE_APPROX(
            wedge.gradient(point),
            (compute_gradient(radius, point, overall_inner_radius,
                              overall_outer_radius, 1.0 - inner_sphericity,
                              1.0 - outer_sphericity)));
      }
    }
  };
...
void test_only_transition() {
  ...
  const double overall_outer_sphericity = 0.0;
  const double this_wedge_inner_radius = 0.5;
  const double this_wedge_outer_radius = 5.0;
  const double this_wedge_inner_sphericity = 1.0;
  const double this_wedge_outer_sphericity = 1.0;
  const OrientationMap<3> orientation_map{};

  const Wedge wedge{overall_inner_radius,
                    overall_outer_radius,
                    overall_inner_sphericity,
                    overall_outer_sphericity,
                    this_wedge_inner_radius,
                    this_wedge_outer_radius,
                    this_wedge_inner_sphericity,
                    this_wedge_outer_sphericity,
                    orientation_map};

  const double overall_inner_distance = 0.5;
  const double overall_outer_distance = overall_outer_radius / sqrt(3.0);
  ...
  const double function_value =
      (overall_outer_distance - 4.0) / distance_difference;
  ...
  set_orig_rad_over_rad(std::array{0.0, -4.0, 0.0}, 0.0);
  CHECK_FALSE(orig_rad_over_rad.has_value());
  // Correct direction, outside wedge
  set_orig_rad_over_rad(std::array{0.0, 0.0, 0.4}, 0.1);
  CHECK_FALSE(orig_rad_over_rad.has_value());
  // At overall inner boundary.
  set_orig_rad_over_rad(std::array{0.0, 0.0, 0.5 * overall_inner_radius}, 0.5);
  CHECK(orig_rad_over_rad.has_value());
...
void test_in_shape_map() {
  ...
  for (const auto& [inner_sphericity, middle_sphericity, outer_sphericity,
    ...
    std::uniform_real_distribution<double> radial_dist{
        this_wedge_inner_radius, this_wedge_outer_radius / sqrt(3.0)};

    std::unique_ptr<ShapeMapTransitionFunction> wedge = std::make_unique<Wedge>(
        overall_inner_radius, overall_outer_radius, inner_sphericity,
        outer_sphericity, this_wedge_inner_radius, this_wedge_outer_radius,
        middle_sphericity, outer_sphericity, orientation);
    ...
    for (size_t i = 0; i < 10; i++) {
      ...
      // The 0.9 factor guarantees that we are inside the mapped outer radius of
      // the inner wedge.
      check_bad_point("Point in inner wedge", overall_inner_radius,
                      0.9 * this_wedge_inner_radius / sqrt(3.0));
      check_bad_point("Too small radius", 0.01,
                      overall_inner_radius / sqrt(3.0));
      check_bad_point("Too big radius", this_wedge_outer_radius * 10.0,
                      this_wedge_outer_radius * 50.0);
    }
  }
}
