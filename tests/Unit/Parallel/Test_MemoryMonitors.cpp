...
void check_output(const std::string& filename, const double time,
  ...
  CHECK(data(0, 0) == time);
  // We can't check the data at this point because it may be a different size
...
void test_monitor_memory_event() {
  ...
  check_output<mem_mon_comp>(outfile_name, time, num_nodes);
  check_output<sing_comp>(outfile_name, time, num_nodes);
