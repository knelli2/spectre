...
void test_wrong_box() {
  ...
  CHECK_THROWS_WITH(
      ([]() {
        db::DataBox<tmpl::list<>> empty_box{};
        Parallel::GlobalCache<metavars> cache{};
        Events::MonitorMemory<3, TimeTag> event{};

        event(0.0, empty_box, cache, 0_st,
              std::add_pointer_t<dg_elem_comp<metavars>>{});
      }()),
      Catch::Contains("The domain::Tags::Element<Dim> tag must be in the "
                      "DataBox to run the MonitorMemory event.\n"));
}
...
void test_monitor_memory_event() {
  ...
  const auto& element_box =
      ActionTesting::get_databox<dg_elem_comp<event_metavars>,
