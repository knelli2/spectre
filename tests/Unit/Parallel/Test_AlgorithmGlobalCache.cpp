...
//
// 4) CheckMemoryMonitorRelatedMethods calls the
 ...
 public:
  ...
  static void execute_next_phase(
    ...
    if (next_phase == Metavariables::Phase::CallMemoryMonitorRelatedMethods) {
      ...
      // time is arbitrary
      auto mutable_cache_proxy = local_cache.mutable_cache_proxy();
      mutable_cache_proxy.compute_size_for_memory_monitor(global_cache, time);
      ...
      mutable_cache_size_ = size_of_object_in_bytes(*Parallel::local_branch(
                                local_cache.mutable_cache_proxy())) /
                            1.0e6;
    } else if (next_phase == Metavariables::Phase::CheckMemoryMonitorFile) {
      // This is ok because we write to disk in the previous phase so all
      // writing is guaranted to have completed by the time we enter this phase.
      auto hdf5_lock =
...
struct TestMetavariables {
  ...
  using component_list =
      tmpl::list<MutateCacheComponent<TestMetavariables>,
                 ...
                 MemoryMonitor<TestMetavariables>,
