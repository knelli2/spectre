...
struct UpdateAggregator {
   ...
   */
  UpdateAggregator(const std::unordered_set<std::string>& control_system_names);
   ...
   * class was constructed with.
   *
   * The sorting is done with `std::sort`.
   */
   ...
   * true, and only be called once. It also must be called before
   * `combined_measurement_expiration_time`. This function leaves all stored
   * data in tact.
   */
  std::unordered_map<std::string, std::pair<DataVector, double>>
   ...
   *
   * \details This function is expected to only be called when `is_ready` is
   * true, and only be called once. It also must be called after
   * `combined_fot_expiration_times`. This function clears all stored data when
   * it is called.
   */
