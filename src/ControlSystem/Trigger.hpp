...
class Trigger : public DenseTrigger {
 ...
 public:
  /// \cond
  explicit Trigger(CkMigrateMessage* const msg) noexcept : DenseTrigger(msg) {}
  ...
  Result is_triggered(
      ...
      const std::unordered_map<
          ...
          measurement_timescales) noexcept {
    ...
    if (triggered) {
      *next_trigger_ += tmpl::as_pack<ControlSystems>(
          [&measurement_timescales, &time](auto... control_systems) noexcept {
            return std::min(
