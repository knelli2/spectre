    ...
    [[maybe_unused]] const auto construct_measurement_timescales =
        [&timescales, &initial_time, &initial_time_step, &function_of_time_file,
         ...
         &measurements_per_update](const auto& option_holder) {
          ...
          // We subtract off one measurement time here because the measurement
          // timescales are only valid for the first N-1 measurements. On the
          // Nth measurement (where an update happens), we don't know the next
          // measurement time because we have to wait for the control systems to
          // update the timescales.
