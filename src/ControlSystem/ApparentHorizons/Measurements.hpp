  ...
  struct FindHorizon : tt::ConformsTo<protocols::Submeasurement> {
   ...
   public:
    ...
    static void apply(
        ...
        const tnsr::ijaa<DataVector, 3, ::Frame::Inertial>& dtphi,
...
struct OneHorizon : tt::ConformsTo<protocols::Measurement> {
  struct FindHorizon : tt::ConformsTo<protocols::Submeasurement> {
   private:
    template <typename ControlSystems>
    struct InterpolationTarget {
      static std::string name() {
        return "ControlSystem::BothHorizons::AhA";
      }
  ...
  using submeasurements = tmpl::list<FindHorizon>;
};
}  // namespace control_system::ah
