...
QuaternionFunctionOfTime<MaxDeriv>::QuaternionFunctionOfTime(
    ...
    : stored_quaternions_and_times_{{t, std::move(initial_quat_func)}},
      omega_f_of_t_(t, initial_omega_func, expiration_time),
...
void QuaternionFunctionOfTime<MaxDeriv>::update(
    double time_of_update, DataVector updated_max_deriv,
    double next_expiration_time) noexcept {
  omega_f_of_t_.update(time_of_update, updated_max_deriv, next_expiration_time);
  update_stored_info();
}

template <size_t MaxDeriv>
void QuaternionFunctionOfTime<MaxDeriv>::update_stored_info() const noexcept {
...
void QuaternionFunctionOfTime<MaxDeriv>::solve_quaternion_ode(
  ...
  const auto quaternion_ode_system =
      [this](const std::array<DataVector, 1>& state,
        ...
        const std::array<DataVector, 1> omega = omega_f_of_t_.func(time);
...
// do explicit instantiation of MaxDeriv = {0,1,2,3}
