...
/// addition to the DomainCreator. First is the initial time step. The
...
struct FunctionsOfTime : db::SimpleTag {
  ...
  static type create_from_options(
      const std::unique_ptr<::DomainCreator<Metavariables::volume_dim>>&
          domain_creator,
      const double initial_time_step, const OptionHolders&&... option_holders) {
    ...
    [[maybe_unused]] const auto lambda = [&initial_expiration_times,
      ...
      initial_expiration_times[name] =
          initial_expiration_time < initial_time_step ? initial_time_step
