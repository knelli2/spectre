 ...
 * a wedge to an outer surface of a wedge. Meant to be used in a
 * `domain::CoordinateMaps::Wedge` block.
 *
 ...
 *
 * \note Since $D$ is only a function of $\theta$ and the shape map only
 * distorts radii and does not affect angles, we have that $D(r,\theta,\phi) =
 * D(\tilde{r},\theta,\phi)$. Therefore we can take $D$ to be constant in Eq.
 * $\ref{eq:r_over_rtil}$ since it won't change with radius.
 *
 * ## Special cases
 *
