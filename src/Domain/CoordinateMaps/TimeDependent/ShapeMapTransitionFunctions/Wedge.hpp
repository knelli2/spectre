 ...
 * \phi)$, the transition function is simply
 *
 * \begin{equation}
 *     \label{eq:interior_transition_func}
 *     G(r,\theta,\phi) = \frac{1}{D_{\text{in}}(r, \theta, \phi)} =
 *     \frac{1}{R_0}
 * \end{equation}
 *
 * and none of the vector formulation is necessary. The second equality is
 * explained by one of the following bullets.
 *
 ...
 * \end{equation}
 *
 * If the axis is `Axis::Interior`, this formula simplifies to (because the
 * inner surface must be spherical)
 *
 * \begin{equation}
 *    \frac{r}{\tilde{r}} =
 *    \begin{cases}
 *      \frac{R_0}{R_0 - \Sigma(\theta,\phi)} & 0 < r <= R_0 \\
 *      1 & r = 0
 *    \end{cases}
 * \end{equation}
 *
 * For other axes, after plugging in $f(r,\theta,\phi)$ and solving, we get
 *
 * \begin{equation}
 ...
 *
 * If the axis is `Axis::Interior`, then the gradient $\partial G/\partial x_i =
 * 0$. For the other axes, we need to compute the gradients of $\vec x_0$ and
 * $\vec x_1$.
 *
