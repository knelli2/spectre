 ...
 * \brief RadialTranslation map defined by \f$\vec{x} =
 * \vec{\xi}+F(r)\vec{T}(t)\f$ where $F(r)$ takes on different forms based on
 ...
 * RadialTranslation Map class is overloaded so that the user can choose between
 * a piecewise translation, radial translation or a uniform translation based on
...
class RadialTranslation {
 public:
  ...
  RadialTranslation(const RadialTranslation<Dim>& radial_translation) = default;
  RadialTranslation& operator=(RadialTranslation&&) = default;
  RadialTranslation& operator=(const RadialTranslation& radial_translation) =
      default;
 ...
 private:
  ...
  friend bool operator==(  // NOLINT(readability-redundant-declaration)
      const RadialTranslation<LocalDim>& lhs,
      const RadialTranslation<LocalDim>& rhs);

  template <typename T>
  std::array<tt::remove_cvref_wrap_t<T>, Dim> function_and_velocity_helper(
      const std::array<T, Dim>& source_coords, double time,
      const std::unordered_map<
          std::string,
          std::unique_ptr<domain::FunctionsOfTime::FunctionOfTime>>&
...
template <size_t Dim>
inline bool operator!=(const RadialTranslation<Dim>& lhs,
