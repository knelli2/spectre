...
BinaryCompactObject::BinaryCompactObject(
  ...
  expiration_times_[expansion_function_of_time_name_] =
      std::numeric_limits<double>::infinity();
  expiration_times_[rotation_about_z_axis_function_of_time_name_] =
      std::numeric_limits<double>::infinity();
  for (auto& name : size_map_function_of_time_names_) {
    expiration_times_[name] = std::numeric_limits<double>::infinity();
  }
}
...
BinaryCompactObject::functions_of_time(
  ...
  if (not initial_expiration_times.empty()) {
    for (auto& [name, expr_time] : initial_expiration_times) {
      ASSERT(initial_expiration_times.count(name) == 1,
             "Could not find function of time '" + name +
                 "' in supplied map of expiration times. Keys of supplied "
                 "expiration times are " +
                 keys_of(initial_expiration_times));
      expiration_times_[name] = expr_time;
    }
  }
        ...
        std::make_unique<FunctionsOfTime::PiecewisePolynomial<3>>(
            ...
            std::array<DataVector, 4>{
                {{gsl::at(initial_size_map_values_, i)},
                 ...
                 {0.0}}},
            expiration_times_[gsl::at(size_map_function_of_time_names_, i)]);
  }
