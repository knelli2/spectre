...
CubicScale<MeshDim>::functions_of_time(
  ...
  if (not initial_expiration_times.empty()) {
    for (size_t i = 0; i < functions_of_time_names_.size(); i++) {
      ASSERT(initial_expiration_times.count(
                 gsl::at(functions_of_time_names_, i)) == 1,
             "Could not find function of time '" +
                 gsl::at(functions_of_time_names_, i) +
                 "' in supplied map of expiration times. Keys of supplied "
                 "expiration times are " +
                 keys_of(initial_expiration_times));
      gsl::at(expiration_times_, i) =
          initial_expiration_times.at(gsl::at(functions_of_time_names_, i));
    }
  }
  ...
  // This is expected.
  for (size_t i = 0; i < functions_of_time_names_.size(); i++) {
    result[functions_of_time_names_[i]] =
