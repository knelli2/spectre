...
class BinaryCompactObject : public DomainCreator<3> {
 ...
 private:
  ...
  // mutable because the expiration times can potentially be changed in the
  // const member `functions_of_time` if there are control systems
  mutable std::unordered_map<std::string, double> expiration_times_{};
  ...
  double initial_expansion_velocity_{
      std::numeric_limits<double>::signaling_NaN()};
  std::string expansion_function_of_time_name_{"Expansion"};
  ...
  double initial_angular_velocity_{
      std::numeric_limits<double>::signaling_NaN()};
  std::string rotation_about_z_axis_function_of_time_name_{"Rotation"};
