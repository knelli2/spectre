...
struct ShapeMapOptions {
 ...
 public:
  ...
  struct Falloff {
    using type = FalloffEnum;
    static constexpr Options::String help = {
        "How the shape map transition function falls off from 1 at the inner "
        "surface to 0 at the outer surface."};
  };
  ...
  using options = tmpl::conditional_t<
      ...
      tmpl::push_back<common_options, TransitionEndsAtCube, Falloff>>;

  size_t l_max{};
  std::array<double, 3> initial_size_values{};
  bool transition_ends_at_cube{false};
  FalloffEnum falloff{FalloffEnum::Inverse};
};
...
struct TimeDependentMapOptions {
 ...
 public:
   ...
   *
   * If the radii for an object are `std::nullopt`, this means that a Size map
