...
FromVolumeFileShapeSize<Object, AllowReplay>::FromVolumeFileShapeSize(
    ...
    : FromVolumeFile<AllowReplay>(std::move(h5_filename),
                                  std::move(subfile_name)),
...
FunctionsOfTimeMap get_shape_and_size(
  ...
  if (std::holds_alternative<FromVolumeFileShapeSize<Object, AllowReplay>>(
          shape_map_options)) {
    const auto& from_vol_file =
        std::get<FromVolumeFileShapeSize<Object, AllowReplay>>(
            shape_map_options);
  ...
  const auto& hard_coded_options = std::get<
      ShapeMapOptions<IncludeTransitionEndsAtCube, Object, AllowReplay>>(
      shape_map_options);
...
#define INCLUDETRANSITION(data) BOOST_PP_TUPLE_ELEM(2, data)

#define INSTANTIATE(_, data)                                                   \
  template struct ShapeMapOptions<INCLUDETRANSITION(data), OBJECT(data),       \
                                  REPLAY(data)>;                               \
  template size_t l_max_from_shape_options(                                    \
      const std::variant<ShapeMapOptions<INCLUDETRANSITION(data),              \
                                         OBJECT(data), REPLAY(data)>,          \
                         FromVolumeFileShapeSize<OBJECT(data), REPLAY(data)>>& \
          shape_map_options);                                                  \
...
#define INSTANTIATE(_, data) \
  template class FromVolumeFileShapeSize<OBJECT(data), REPLAY(data)>;
