 ...
 * from the `Cce::Tags::worldtube_boundary_tags_for_writing` list.
 */
...
class WorldtubeModeRecorder {
 public:
  ...
  /*!
   * \brief Given a tag name, get the subfile path in the H5 file.
   *
   * \details Returns `Cce::dataset_label_for_tag` with a "/" at the beginning.
   */
  template <typename Tag>
  static std::string subfile_name();
