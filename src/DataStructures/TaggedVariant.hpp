...
struct compare_if_same_tag {
  ...
  constexpr bool operator()(const T& /*a*/, const U& /*b*/) {
    ERROR("comparison implementation bug");
    return false;  // return statement required in constexpr function
  }
};
}  // namespace TaggedVariant_detail
...
constexpr bool operator==(const TaggedVariant<Tags...>& a,
  ...
  return a.index() == b.index() and
         (a.valueless_by_exception() or
          visit(TaggedVariant_detail::compare_if_same_tag<std::equal_to<>>{}, a,
                b));
}
template <typename... Tags>
constexpr bool operator!=(const TaggedVariant<Tags...>& a,
...
constexpr bool operator<(const TaggedVariant<Tags...>& a,
                         const TaggedVariant<Tags...>& b) {
  if (UNLIKELY(b.valueless_by_exception())) {
    return false;
  }
  if (UNLIKELY(a.valueless_by_exception())) {
    return true;
  }
  return a.index() < b.index() or
         (a.index() == b.index() and
          visit(TaggedVariant_detail::compare_if_same_tag<std::less<>>{}, a,
                b));
}
template <typename... Tags>
constexpr bool operator>(const TaggedVariant<Tags...>& a,
