...
ylm::Strahlkorper<Frame> read_surface_ylm_single_time(
    ...
    const double time, const double epsilon) {
  ...
  const size_t num_rows = static_cast<size_t>(dimensions[0]);
  const Matrix times = ylm_file.get_data_subset(columns, 0, num_rows);

  std::optional<size_t> row_number{};
  for (size_t i = 0; i < num_rows; i++) {
    if (equal_within_roundoff(time, times(i, 0), epsilon, time)) {
      if (row_number.has_value()) {
        ERROR("Found two times in the subfile "
              << surface_subfile_name << " of the H5 file " << file_name
              << " that are within an epsilon of " << epsilon
