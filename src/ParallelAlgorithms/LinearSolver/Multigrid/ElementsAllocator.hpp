  ...
  static void apply(Parallel::CProxy_GlobalCache<Metavariables>& global_cache,
    ...
    do {
      ...
      const domain::BlockZCurveProcDistribution<Dim> element_distribution{
          static_cast<size_t>(num_of_procs_to_use), initial_refinement_levels,
          procs_to_ignore};
      ...
      Parallel::printf(
          ...
          "procs.\n",
          pretty_type::name<OptionsGroup>(), multigrid_level, element_ids.size(),
          domain.blocks().size(), num_of_procs_to_use);
      ++multigrid_level;
