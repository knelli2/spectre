 ...
 * or nodegroup component, computes the average memory usage per node for each,
    ...
    if constexpr (db::tag_is_retrievable_v<tag, db::DataBox<DbTags>>) {
      db::mutate<tag>(
          ...
          [&cache, &time, &node_or_proc, &size_in_MB](
            ...
            // unordered map
            if (array_memory_holder->count(time) == 0) {
              array_memory_holder->emplace(
                  time,
                  std::unordered_map<int, double>{{node_or_proc, size_in_MB}});
            } else {
              // otherwise continue to add to the existing time
              array_memory_holder->at(time)[node_or_proc] = size_in_MB;
            }
            ...
            const int num_nodes =
                Parallel::number_of_nodes(*mem_monitor_proxy.ckLocal());
            const int num_procs =
                Parallel::number_of_procs(*mem_monitor_proxy.ckLocal());
            const size_t expected_number =
                static_cast<size_t>(is_group ? num_procs : num_nodes);
            ASSERT(array_memory_holder->at(time).size() <= expected_number,
                   "ContributeMemoryData received more data that it was "
                   "expecting. Was expecting "
                       << expected_number << " calls but instead got "
                       << array_memory_holder->at(time).size());
            if (array_memory_holder->at(time).size() == expected_number) {
              ...
              // over the procs to get the average on that node and get the proc
              ...
              for (int node = 0; node < num_nodes; node++) {
                ...
                if (not is_group) {
                  size_on_node = array_memory_holder->at(time).at(node);
                } else {
                  const int first_proc = Parallel::first_proc_on_node(
                      node, *mem_monitor_proxy.ckLocal());
                  ...
                  for (int proc = first_proc; proc < last_proc; proc++) {
                    size_on_node += array_memory_holder->at(time).at(proc);
                    if (array_memory_holder->at(time).at(proc) >
                        max_usage_on_proc) {
                      max_usage_on_proc =
                          array_memory_holder->at(time).at(proc);
                      proc_of_max = proc;
                    }
                  }
                }
              ...
              auto finished_time_iter = array_memory_holder->find(time);
              array_memory_holder->erase(finished_time_iter);
            }
          });
    } else {
      ERROR(
...
struct ProcessNodegroup {
  ...
  static void apply(db::DataBox<DbTags>& /*box*/,
    ...
    const int my_node = Parallel::my_node(*nodegroup_proxy.ckLocalBranch());

    const double size_in_bytes = static_cast<double>(
        size_of_object_in_bytes(*nodegroup_proxy.ckLocalBranch()));
...
struct ProcessGroup {
  ...
  static void apply(db::DataBox<DbTags>& /*box*/,
    ...
    auto& group_proxy =
        Parallel::get_parallel_component<ParallelComponent>(cache);
    const int my_proc = Parallel::my_proc(*group_proxy.ckLocalBranch());

    const double size_in_bytes = static_cast<double>(
        size_of_object_in_bytes(*group_proxy.ckLocalBranch()));
...
struct ProcessSingleton {
  ...
  static void apply(db::DataBox<DbTags>& /*box*/,
    ...
    Parallel::threaded_action<
        ...
        std::make_tuple(time, Parallel::my_proc(*singleton_proxy.ckLocal()),
                        size_in_MB));
  }
};
}  // namespace mem_monitor
