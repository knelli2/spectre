 ...
 * monitor, look in the metavariables for that executable. If you request to
 * monitor an invalid parallel component, an ERROR will occur and a list of the
 * available components to monitor will be printed.
 *
 ...
 public:
  ...
  MonitorMemory(
    ...
    if (components_to_monitor.has_value()) {
      ...
      // 'All' was specified
      for (const auto& name_and_chare : existing_components) {
        components_to_monitor_.insert(name_and_chare.first);
      }
    }
  }
...
void MonitorMemory<Dim, ObservationValueTag>::operator()(
  ...
  if constexpr (db::tag_is_retrievable_v<domain::Tags::Element<Dim>,
        ...
        if (designated_element) {
          ...
          if constexpr (std::is_same_v<component,
            ...
            auto mutable_global_cache_proxy =
                cache.mutable_global_cache_proxy();
    ...
    (void)array_index;
    ERROR(
        "The domain::Tags::Element<Dim> tag must be in the DataBox to run the "
        "MonitorMemory event.\n");
  }
}
